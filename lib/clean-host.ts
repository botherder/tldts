...
export default function extractHostname(url: string): string | null {
    ...
    if (indexOfProtocol !== -1) {
      ...
      for (let i = 0; i < indexOfProtocol; i += 1) {
        ...
        if (!(
          ...
          (lowerCaseCode >= CHARACTERS.ZERO && lowerCaseCode <= CHARACTERS.NINE) || // digit
  ...
  if (url.charCodeAt(start) === CHARACTERS.OPEN_BRACKET) {
    ...
    return null;
  } else if (indexOfPort !== -1 && indexOfPort > start && indexOfPort < end) { // Detect port: ':'
    end = indexOfPort;
  }

  // Trim trailing dots
  for (; end > (start + 1) && url.charCodeAt(end - 1) === CHARACTERS.DOT; end -= 1) { /* */ }
